...
def parse_cfg(cfgfile):
    # open the cfgfile and red it.
    with open(cfgfile, 'r') as file:
        # and remove unnecessary characters like '\n' and '#'. The variable 'lines' is holding all the lines of the file.
...
def YOLOv3Net(cfgfile, model_size, num_classes):
    # call the parse_cfg() and store all the return attributes in a variable blocks
    ...
    # define input model using Keras function and
    inputs = input_image = Input(shape=model_size)
    # devided by 255 to normalize it to the range of 0-1
    inputs = inputs / 255.0
    ...
    for i, block in enumerate(blocks[1:]):
        ...
        if block["type"] == "convolutional":
            ...
            # verify whether the stride is greater than 1, if true, then down sampling is performed,
            # so we need to adjust the padding
            if strides > 1:
